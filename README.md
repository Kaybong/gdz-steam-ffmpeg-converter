# GDZ Steam FFmpeg Converter — Railway v1.2.1
...
Успешный ответ: полностью сформированный и проверенный бинарный `video/mp4` с `Content-Length`.
...
- Итоговый файл ограничен профилем: 48 MiB для Telegram и 42 MiB для Web HQ.
...
- MP4 сначала полностью создаётся во временном файле, затем проверяется FFprobe и только после этого передаётся клиенту с точным `Content-Length`.
- Для Web HQ проверяются длительность, H.264 `1280×720`, размер и минимальный фактический видеобитрейт `900 кбит/с`; неполный файл никогда не возвращается как успешный.
...
- Устранён ложноположительный успех, при котором оборванный fragmented MP4 оставался читаемым и мог быть сохранён n8n как готовый файл.
- Web-профиль переведён с `medium` на `fast` при прежней цели `36 MiB`, пределе `42 MiB` и расчётном высоком битрейте. Это сокращает время кодирования без возврата к низкому web-v1 битрейту.
- Конвертация выполняется во временный обычный MP4 с `+faststart`; до HTTP-ответа FFprobe обязательно сравнивает длительность с исходником с допуском не более `max(2,5 с; 2%)`.
- Добавлены проверки размера, H.264 `1280×720` и минимального фактического web-битрейта `900 кбит/с`.
- Успешный ответ содержит точный `Content-Length` и диагностические заголовки длительности/битрейта. При обрыве клиент не сможет принять частичный файл как полноценный ответ.
- Telegram-профиль, целевые размеры и публичный API `/convert` сохранены; изменён только безопасный способ формирования ответа.
